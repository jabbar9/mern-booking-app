...
test("should edit hotel", async ({ page }) => {
  ...
  await expect(page.locator('[name="name"]')).toHaveValue("Taj Hotel")
  await page.locator('[name="name"]').fill("Taj Hotel Updated")
  await page.getByRole("button", { name: "Save"}).click();
  await expect(page.getByText("Hotel Saved!")).toBeVisible();

  await page.reload();

  await expect(page.locator('[name="name"]')).toHaveValue("Taj Hotel Updated");
  await page.locator('[name="name"]').fill("Taj Hotel");
  await page.getByRole("button", { name: "Save"}).click();
})